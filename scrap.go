...
func setupCollectors(c *colly.Collector, links *[]string, articles *[]Article, mu *sync.Mutex, wg *sync.WaitGroup, pageCount *int, linkCount *int, limiter *rate.Limiter) {
	setupMainCollector(c, links, pageCount)
...
func setupMainCollector(c *colly.Collector, links *[]string, pageCount *int) {
...
func parseArticle(e *colly.HTMLElement) *Article {
	...
	if len(matches) < 2 {
		log.Printf("No se pudo encontrar el categoryId para el artículo: %s", title)
		return nil
	}
